...
### Key is movieID,
### Value is movie title

def makeMovieList():
    inFile = open ("movies.csv", "r")

    ### Create a dictionary to hold movie information:
    movieList = {}
    ### Use a list to store the separate words from the
    ### line read in from the text file:
    line = []
    ### read from file, one line at a time
    string = inFile.readline()
    ### words are strings separated by commas
    word = ""

    ### Parse the line into tokens:
    ### read each line, one character at a time,
    ### adding each character to our "Word"
    ### If a comma is encountered, copy that full "word"
    ### into our list
    ### once we reach the end of the line,
    ### add the movieID and title to movieList
    ### as Key and Value:
...
### end makeMovieList function
    ...
    ### This is a sparse vector of users for each movie,
    ### For each movie, there is a vector sized as the
    ### number of users,
    ### for each user, for each movie that use has rated,
    ### add that rating to that movie using the userID as
    ### the index
...
def scoreMovies(movieDict):
    ...
    for x in range (100): ### shorter list for testing
        newTime = time.time()
        print ("Time: ", newTime - currentTime)
        currentTime = newTime
        ...
        #for y in range (x + 1, len(movieList)): ### this is the real line
            ...
            if theScore > 0:
                ...
                ### Only keep the top 5 highest scoring movies
                ### in descending order (bubblesort):
...
def estimateRatings(userList, scoreList, movieDict, maxUserId):
    ...
    ### for each movie, get the list of ratings:
    ### for each user, if that user hasn't rated that movie,
    ### get the top 5 movies similar to that movie,
    ### If the user has rated any of the movies similar
    ### to that movie, get the average of all movies
    ### similar to this movie that the user has rated
    ### This is the userRating for this movie
