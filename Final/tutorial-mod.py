...
def test_pipeline(df, nlp_pipeline, pipeline_name=''):
    ...
    print pipeline_name + ' mean log loss: ' + str(round(pd.np.mean(losses), 3))
...
#logit_all_features_pipe = Pipeline([('uni', UnigramPredictions()),('nlp', PartOfSpeechFeatures()),('clean', DropStringColumns()), ('clf', LogisticRegression())])
logit_all_features_pipe = Pipeline([('clf', LogisticRegression())])
test_pipeline(train_df, logit_all_features_pipe)
'''

'''
categories = ['MWS', 'EAP', 'HPL']
...
#twenty_train = pd.read_csv("CourseProject/train/train.csv")
twenty_train = pd.read_csv("CourseProject/train/train.csv", usecols=[TEXT_COLUMN, Y_COLUMN])
