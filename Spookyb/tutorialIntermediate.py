# -*- coding: utf-8 -*-
"""
Created on Tue May  1 21:22:54 2018
...
train_df = pd.read_csv("train/train_small.csv", usecols=[TEXT_COLUMN, Y_COLUMN])
test_df = pd.read_csv("test/test_small.csv", usecols=[TEXT_COLUMN])
...
def test_pipeline(df, nlp_pipeline, pipeline_name=''):
    ...
    for train_index, test_index in rskf.split(X, y):
        ...
        y_train, y_test = y[train_index], y[test_index]
        nlp_pipeline.fit(X_train, y_train)
        losses.append(metrics.log_loss(y_test, 
        nlp_pipeline.predict_proba(X_test)))
'''

#generate_submission_df (logit_all_features_pipe, test_df)
#myFile = generate_submission_df (logit_all_features_pipe2, test_df)
generate_submission_df (logit_all_features_pipe, test_df)